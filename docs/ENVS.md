...
| `ABLATE` | Internal. Set automatically when `ABLITERATED=1`. Do not set `ABLATE=1` alone. `prepare --abliterated` reuses a valid `$HF_CACHE/dspark-ablation/direction_r1.pt`; when absent it runs `hf download --include "ablit/*"` against the gated [`drowzeys/keys-DeepSeekV4-Flash-GA-0731-Dspark-Abliterated-Anchored-Tensors`](https://huggingface.co/drowzeys/keys-DeepSeekV4-Flash-GA-0731-Dspark-Abliterated-Anchored-Tensors/tree/main/ablit) repo. First download requires accepting/requesting access plus `hf auth login` or `HF_TOKEN`; failures print the access URL and login steps. The selected `refusal_direction_r1.pt` is the tested 4096-dimensional `broad` direction. The larger `refusal_direction_reablit_20260726.pt` per-layer set is downloaded but not consumed by this single-direction hook. |
